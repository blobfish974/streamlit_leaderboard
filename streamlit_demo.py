...
def load_data(nrows):
    data = pd.read_csv(
        'D:/programming/projects/root_me_leaderboard/train.csv', nrows=nrows)
    return data
...
def load_center_data(nrows):
    data = pd.read_csv(
        'D:/programming/projects/root_me_leaderboard/fulfilment_center_info.csv', nrows=nrows)
    return data
...
def load_meal_data(nrows):
    data = pd.read_csv(
        'D:/programming/projects/root_me_leaderboard/meal_info.csv', nrows=nrows)
    return data
...
df = pd.DataFrame(weekly_data[:200], columns=[
...
chart_data = pd.DataFrame(weekly_data[:40], columns=[
                          'num_orders', 'base_price'])
